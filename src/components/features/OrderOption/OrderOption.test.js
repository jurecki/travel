...
describe('Component OrderOption', () => {
  ...
  it('should return corect title name', () => {
    const expectTitle = 'Travel';
    const component = shallow(<OrderOption type='checkboxes' name={expectTitle}/>);
    expect(component.find('.title').text()).toEqual(expectTitle);
  });
    ...
    switch (type) {
      ...
      case 'number': {
        it ('contains input with type number', () => {
          const number = renderedSubcomponent.find({ type: 'number' });
      ...
      case 'text': {
        it ('contains input with type text', () => {
          const number = renderedSubcomponent.find({ type: 'text' });
          expect(number.length).toBe(1);
        });
